...
```
Initially, the ``.mp4`` file that is in the static folder is transmitted by a stream simulator that is exposed on the ``/stream`` port. The detection results are presented in the ``/home`` route.
